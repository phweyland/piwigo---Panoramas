...
(function($) {
	$.fn.panorama = function(options) {
		this.each(function(){ 
			...
			var mathRound = 100;
			var panoramaThumbHeight = 30;
			var ratio = Math.round((elemHeight * mathRound) / panoramaThumbHeight);
			var panoramaThumbWidth = Math.floor((elemWidth / ratio) * mathRound);
			var FooterHeight = 46;
...
//			}
			var halfFrame = Math.floor((settings.viewport_width / ratio) * mathRound / 2);
			// --------------------------------------------------
...
//					.css('height',FooterHeight)
				...
				$("#panoramaThumb").css('height', (panoramaThumbHeight + 1)).css('width', panoramaThumbWidth).append("<img src='" + $(this).attr('src') + "' width=" + panoramaThumbWidth + " height=" + (panoramaThumbHeight + 1) + "></img><div id='thumbFrame'></div>");

				if(window.ie) { var ieheightadjust = 1;	} else { var ieheightadjust = 4; }
				$("#thumbFrame").css('position','relative')
					.css('width', (Math.floor((settings.viewport_width / ratio) * mathRound) + 1))
					.css('height', (panoramaThumbHeight + 2))
					.css('top', -(panoramaThumbHeight + ieheightadjust + 1));
				
				$("#thumbFrame" ).draggable({
					axis : "x",
					containment: "#panoramaThumb",
					drag: function( event, ui ) {
						var x = Math.floor((ui.position.left * ratio) / mathRound);
						$(".panorama-container").css('margin-left', - x);
					}
				}); 
				
// desactivate because doesn't work properly				
//				$("#panoramaThumb").bind('click', function(e) {
//					var x = Math.floor((e.pageX - this.offsetLeft) - halfFrame);
//					if(x < 0) { 
//						x = 0;
//					}
//					else if (x > panoramaThumbWidth - $("#thumbFrame").width()) {
//						x = panoramaThumbWidth - $("#thumbFrame").width();
//					}
...
// Gestion des boutons de contrôle du panorama
			...
			panoramaViewport.find('a.panorama-control-left').bind('click', function() {
				...
				panorama_animate(panoramaContainer, elemWidth, settings, ratio / mathRound);
				return false;
			});
		...
		if (settings.auto_start) panorama_animate(panoramaContainer, elemWidth, settings, ratio / mathRound);
	});

/**************************************************************/
// Scroll horizontal du panorama		
		function panorama_animate(element, elemWidth, settings, rapport) {
			...
			if (settings.direction == 'right') {
				$(element).animate({marginLeft: 0}, ((settings.speed / elemWidth) * (currentPosition)) , 'linear', function (){ 
					if (settings.footer_display == 'visible') panorama_stop(element, rapport);
					if (settings.mode_360) {
						$(element).css('marginLeft', '-'+(parseInt(parseInt(elemWidth))+'px'));
						panorama_animate(element, elemWidth, settings, rapport);
